...
void IndexedModel::CalcNormals()
{
    for(unsigned int i = 0; i < indices.size(); i += 3)
    {
		...
		glm::vec3 v3 = positions[i3] - positions[i0];
...
static inline std::vector<std::string> SplitString(const std::string &s, char delim)
{
	...
	unsigned int i;
	std::vector<std::string> arrRets;
	const char *pSrc =s.c_str(); unsigned int uSrcLen =s.length(); 
	unsigned int uSubStart =0xffffffff;
	for(i =0; i<uSrcLen; i++){
		if(uSubStart == 0xffffffff){
			if(pSrc[i] != delim){uSubStart =i;}
		}else{
			if(pSrc[i] == delim){arrRets.push_back(s.substr(uSubStart, i-uSubStart)); uSubStart =0xffffffff;}
		}
	}
	if(uSubStart != 0xffffffff){
		arrRets.push_back(s.substr(uSubStart, uSrcLen-uSubStart));
	}
	return arrRets;
}
